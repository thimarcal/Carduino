...
void CarHandler::init() {}
void CarHandler::handle(String command) {
  if (command.length() <= 0)
    return;
...
void CarHandler::executeAction(char action, int power) {
  switch (action) {
    case 'f':
      go(FORWARD, power);
      break;
...
void CarHandler::go(int direction, int power) {
...
void CarHandler::turn(int direction, int power) {
  if (direction == RIGHT) {
